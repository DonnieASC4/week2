...
/*
Create a function named NumAddInt that takes an integer as an argument. Add up all the numbers from 1 to the number you passed to the function. For example, if the input is 4 then your function should return 10 because 1 + 2 + 3 + 4 = 10.
*/
...
Parameters: 
num - a number of minutes (num must be greater than 0 (num >= 0))
...
1:3 (Where 1 is the number of hours and 3 is the number of minutes)
...
let sentence = "I am sitting in the classroom and chilling."
...
Write a function that takes in an array of random numbers and returns an array with numbers 
in the range of 2 numbers called lowBound and highBound.

All elements in the returned array should be betwee lowBound < element < highBound. [Not including numbers with the value lowBound and highBound]

Parameters:
arr - an array of random numbers (will only have numbers)
lowBound - a number (lowBound can be any number: positive, negative, or 0)
highBound - a number (highBound can be any number: positive negative, or 0)

Return:
rangeArr - an array whose numbers who range between lowBound and highBound


 [The order doesn't matter for rangeArr]

Ex.)

let arr = [1, 2, 3, 4, 5, -1];
let smallerArr = rangeArray(arr, -2, 3);
